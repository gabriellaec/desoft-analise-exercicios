    ...
    if aposta <= 0 or aposta > dinheiro:
        ...
        if tipo_aposta == 'n':
            ...
            if escolha_numero == numero_sorteado:
                dinheiro = aposta *35
                print('Parabens, voce ganhou! Seu saldo é de {} dinheiros'.format(dinheiro))
                break
            else:
                dinheiro -= aposta
                print('Voce perdeu! Seu saldo é de {} dinheiros'.format(dinheiro))
                break
            ...
            if escolha_par_impar == 'p':               
                ...
                if numero_sorteado_2 %2 != 0:
                    ...
                    dinheiro -= aposta
                    print('Voce perdeu! Seu saldo é de {} dinheiros'.format(dinheiro))
                    breakput('Quanto voce deseja apostar? '))
    if aposta <= 0 or aposta > dinheiro:
        break
    else:
        tipo_aposta = input('Voce deseja apostar em um numero (n) ou disparidade (p)? ')
